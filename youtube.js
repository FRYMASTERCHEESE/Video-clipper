...
const els = {
  ...
  channelDescription: $('channelDescriptionDraft'), generateDescription: $('generateChannelDescription'), applyDescription: $('applyChannelDescription'),
...
function setConnectedControls(enabled) {
  [els.refresh, els.buildPlan, els.loadVideos, els.autoFixMetadata, els.generateDescription, els.applyDescription].forEach(el => el.disabled = !enabled);
  refreshUploadState();
}
...
els.applyDescription.addEventListener('click', async () => {
  ...
  try {
    ...
    preservedChannelBranding.description = description;
    ...
    state.channel.brandingSettings = { ...(state.channel.brandingSettings || {}), channel: { ...currentChannelBranding, description } };
